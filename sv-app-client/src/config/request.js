	...
	useRequest() {
		...
		if (this.options.url.toLowerCase().startsWith("http")) {
			...
			if (this.options.url.startsWith("/")) {
				result.url = defaultOptions.baseURL + this.options.url
			} else {
				// 不以/开头则自动拼接上/
				result.url = defaultOptions.baseURL + '/' + this.options.url
			}
		...
		result.method = this.options.method.toUpperCase() || defaultOptions.method
		result.responseType = this.options.responseType || defaultOptions.responseType || ''
		result.timeout = this.options.timeout || defaultOptions.timeout

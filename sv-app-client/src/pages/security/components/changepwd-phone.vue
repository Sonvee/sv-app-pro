    ...
    <view class="changepwd-form">
      <uni-forms ref="changepwdFormRef" :model="changepwdForm" :rules="changepwdRules">
        <uni-forms-item name="phone">
          <uni-easyinput v-model="changepwdForm.phone" type="text" :maxlength="11" placeholder="请输入手机号" prefixIcon="phone" />
        </uni-forms-item>
...
import { smsCaptcha } from '@/api/auth'
import { changePasswordByPhone } from '@/api/user/user'
...
function getCaptchaPhone() {
  // 手机号合法性校验
  const phoneRegExp = useRegExp('phone')
  if (!phoneRegExp.regexp.test(changepwdForm.value.phone)) {
    uni.showToast({
      title: phoneRegExp.msg,
      icon: 'none'
    })
    return
  }

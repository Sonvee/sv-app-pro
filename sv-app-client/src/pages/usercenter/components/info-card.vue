  ...
  <view class="info-card">
    ...
    <view class="user-intro">
      <view class="flex justify-between">
        <text class="text-bold text-xl">{{ userInfo?.username }}</text>
        <text class="text-blue text-sm" @click="isFold = !isFold">展开</text>
      </view>
      <view class="margin-top-xs text-cyan">
        {{ userInfo?.comment || '这个人很神秘，什么都没有写' }}
      </view>
      <view v-if="!isFold">
        <view class="margin-top-sm text-grey text-sm">
          <i class="uni-icons-map-pin-ellipse margin-right-xs"></i>
          <text>IP归属地：{{ userInfo?.login_ip }}</text>
        </view>

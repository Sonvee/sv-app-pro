	...
	<sv-page enablePullDownRefresh @onRefresh="onPullDown">
		<view class="vip-page">
			...
			<view class="sv-grid grid-col-5 padding-lr-sm">
				<view class="grid-item-xl flex-col align-center justify-evenly text-tyblue" v-for="item in benefits" :key="item.lable">
			...
			<!-- 底部栏 -->
			<pay-info v-if="curSubscriptionWay == 'wallet'" :info="curPlan" @success="getVipInfo"></pay-info>
		</view>
	</sv-page>
</template>
...
async function getVipInfo() {
	...
	// 分发会员订阅信息
	uni.$emit('E_VIP_INFO', vipInfo.value)
}
...
function selectPlan(e) {
	curPlan.value = e
	benefits.value = e.benefits_detail
}

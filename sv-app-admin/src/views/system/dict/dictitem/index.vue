  ...
  <div class="page-container">
    ...
    <div class="card table-container">
      ...
      <div class="table-control">
        <el-button type="primary" plain :icon="Plus" v-permission="['sys:dictitem:add']" @click="add">新增</el-button>
        <el-button type="danger" plain :icon="Delete" v-permission="['sys:dictitem:batchdelete']" :disabled="!isTruthy(batchSelection, 'arr')" @click="batchDelete">
          批量删除
        </el-button>
        <div style="flex: 1"></div>
        <ExcelTool ref="excelToolRef" class="mr-12" v-permission="['sys:dictitem:excel']" @onTool="onExcelTool" @confirmUpload="excelUpload"></ExcelTool>
      ...
      <el-table v-loading="loading" :data="tableData" border @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="action_style" label="标签样式" align="center" width="100" show-overflow-tooltip>
          <template #default="scope">
            <el-tag v-if="scope.row.action_style" :type="scope.row.action_style">{{ scope.row.action_style }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="160" fixed="right">
          <template #default="scope">
            <el-button-group>
              <el-button text type="primary" :icon="EditPen" v-permission="['sys:dictitem:update']" @click="edit(scope.row)">编辑</el-button>
              <el-button text type="danger" :icon="Delete" v-permission="['sys:dictitem:delete']" @click="del(scope.row)">删除</el-button>
            </el-button-group>
          </template>
        </el-table-column>
      </el-table>
...
async function handleTable(params) {
  loading.value = true
  const res = await dictitemList(params)
...
// 增
function add() {
  formInit.value = { dict_type: dictType.value } // 参数初始化
  formMode.value = 'add'
  showForm.value = true
}
...
// 删
function del(row) {
  const { dictitem_id, label } = row
  ElMessageBox.confirm(`确认删除『 ${label} 』吗？`, '系统提示', {
    ...
    .then(async () => {
      // 确认删除操作
      const deleteRes = await dictitemDelete({ dictitem_id })
      ElMessage({
        type: 'success',
...
function handleSelectionChange(e) {
  batchSelection.value = e.map((item) => item.label)
}
...
function batchDelete() {
  ...
  ElMessageBox.confirm(`确认批量删除所选项吗？`, '系统提示', {
    ...
    .then(async () => {
      // 确认批量删除操作
      const deleteRes = await dictitemBatchDelete({
...
async function submitForm(e) {
  try {
    ...
    switch (e.mode) {
      case 'add':
        // 新增添加
        result = await dictitemAdd(e.data)
        break
      case 'edit':
        // 编辑更新
        result = await dictitemUpdate(e.data)
        break
    }
    if (result.success) {
